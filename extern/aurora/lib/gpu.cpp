...
static logwrapper::Module Log("aurora::gpu");
...
static void error_callback(WGPUErrorType type, char const* message, void* userdata) {
  Log.report(logwrapper::Fatal, FMT_STRING("Dawn error {}: {}"),
...
static void device_callback(WGPURequestDeviceStatus status, WGPUDevice device, char const* message, void* userdata) {
  if (status == WGPURequestDeviceStatus_Success) {
    ...
    Log.report(logwrapper::Warning, FMT_STRING("Device request failed with message: {}"), message);
  }
...
bool initialize(SDL_Window* window, wgpu::BackendType backendType, uint32_t msaa, uint16_t aniso) {
  if (!g_Instance) {
    Log.report(logwrapper::Info, FMT_STRING("Creating Dawn instance"));
    g_Instance = std::make_unique<dawn::native::Instance>();
  }
  Log.report(logwrapper::Info, FMT_STRING("Attempting to initialize {}"), magic_enum::enum_name(backendType));
#if 0
  ...
  Log.report(logwrapper::Info, FMT_STRING("Graphics adapter information\n  API: {}\n  Device: {} ({})\n  Driver: {}"),
  ...
  Log.report(logwrapper::Info, FMT_STRING("Using swapchain format {}"), magic_enum::enum_name(swapChainFormat));
  {
