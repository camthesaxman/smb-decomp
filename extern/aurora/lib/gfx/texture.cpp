...
static TextureFormatInfo format_info(wgpu::TextureFormat format) {
  ...
  case wgpu::TextureFormat::BC1RGBAUnorm:
    return {4, 4, 8, true};
  default:
    Log.report(logwrapper::Fatal, FMT_STRING("format_info: unimplemented format {}"), magic_enum::enum_name(format));
    unreachable();
  }
}
...
TextureHandle new_static_texture_2d(uint32_t width, uint32_t height, uint32_t mips, GXTexFmt format,
  ...
  for (uint32_t mip = 0; mip < mips; ++mip) {
    ...
    if (offset + dataSize > data.size()) {
      Log.report(logwrapper::Fatal, FMT_STRING("new_static_texture_2d[{}]: expected at least {} bytes, got {}"), label,
...
//    g_textureUploads.emplace_back(dataLayout, std::move(dstView), physicalSize);
  ...
  if (data.size() != UINT32_MAX && offset < data.size()) {
    Log.report(logwrapper::Warning, FMT_STRING("new_static_texture_2d[{}]: texture used {} bytes, but given {} bytes"),
               label, offset, data.size());
  }
  return handle;
}

TextureHandle new_dynamic_texture_2d(uint32_t width, uint32_t height, uint32_t mips, GXTexFmt format,
...
TextureHandle new_render_texture(uint32_t width, uint32_t height, GXTexFmt fmt, zstring_view label) noexcept {
...
void write_texture(const TextureRef& ref, ArrayRef<uint8_t> data) noexcept {
  ...
  for (uint32_t mip = 0; mip < ref.mipCount; ++mip) {
    ...
    const uint32_t dataSize = bytesPerRow * heightBlocks * mipSize.depthOrArrayLayers;
    if (offset + dataSize > data.size()) {
      Log.report(logwrapper::Fatal, FMT_STRING("write_texture: expected at least {} bytes, got {}"), offset + dataSize,
                 data.size());
      unreachable();
    }
//    auto dstView = wgpu::ImageCopyTexture{
...
//    g_textureUploads.emplace_back(dataLayout, std::move(dstView), physicalSize);
  ...
  if (data.size() != UINT32_MAX && offset < data.size()) {
    Log.report(logwrapper::Warning, FMT_STRING("write_texture: texture used {} bytes, but given {} bytes"), offset,
               data.size());
  }
}
} // namespace aurora::gfx
