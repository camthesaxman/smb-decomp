...
namespace aurora {
static logwrapper::Module Log("aurora");
...
static void set_window_icon(Icon icon) noexcept {
  ...
  if (iconSurface == nullptr) {
    Log.report(logwrapper::Fatal, FMT_STRING("Failed to create icon surface: {}"), SDL_GetError());
    unreachable();
  }
...
static void resize_swapchain(bool force) noexcept {
  ...
  if (size.scale != g_windowSize.scale) {
    if (g_windowSize.scale > 0.f) {
      Log.report(logwrapper::Info, FMT_STRING("Display scale changed to {}"), size.scale);
    }
...
static bool poll_events() noexcept {
  ...
  while (SDL_PollEvent(&event) != 0) {
    ...
    // Log.report(logwrapper::Info, FMT_STRING("Received SDL event: {}"), event.type);
  }
  return true;
}
...
void app_run(std::unique_ptr<AppDelegate> app, Icon icon, int argc, char** argv, std::string_view configPath,
  ...
  if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
    Log.report(logwrapper::Fatal, FMT_STRING("Error initializing SDL: {}"), SDL_GetError());
    unreachable();
  }
  ...
  if (g_window == nullptr) {
    Log.report(logwrapper::Fatal, FMT_STRING("Error creating window: {}"), SDL_GetError());
    unreachable();
  }
  ...
  if (gpu::g_backendType == wgpu::BackendType::Null) {
    ...
    if (renderer == nullptr) {
      Log.report(logwrapper::Fatal, FMT_STRING("Failed to initialize SDL renderer: {}"), SDL_GetError());
      unreachable();
    }
  }
  ...
  imgui::create_context(configPath);
  const auto size = get_window_size();
  Log.report(logwrapper::Info, FMT_STRING("Using framebuffer size {}x{} scale {}"), size.fb_width, size.fb_height,
             size.scale);
  ...
  Log.report(logwrapper::Info, FMT_STRING("Application exiting"));
