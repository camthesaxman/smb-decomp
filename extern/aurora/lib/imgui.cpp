...
ImTextureID add_texture(uint32_t width, uint32_t height, uint8_t* data) noexcept {
  if (g_useSdlRenderer) {
    SDL_Renderer* renderer = ImGui_ImplSDLRenderer_GetBackendData()->SDLRenderer;
  ...
  const auto textureDescriptor = WGPUTextureDescriptor{
      .label = "imgui texture",
      .usage = WGPUTextureUsage_StorageBinding | WGPUTextureUsage_CopyDst,
...
// C bindings
ImTextureID aurora_imgui_add_texture(uint32_t width, uint32_t height, void* rgba8) {
  return aurora::imgui::add_texture(width, height, static_cast<uint8_t*>(rgba8));
}
