...
struct AppDelegate {
  ...
  // Input
  virtual void onCharKeyDown(u8 charCode, ModifierKey mods, bool isRepeat) noexcept = 0;
  virtual void onCharKeyUp(u8 charCode, ModifierKey mods) noexcept = 0;
  ...
  // Controller
  virtual void onControllerAdded(u32 which) noexcept = 0;
  virtual void onControllerRemoved(u32 which) noexcept = 0;
  virtual void onControllerButton(u32 which, ControllerButton button, bool pressed) noexcept = 0;
  virtual void onControllerAxis(u32 which, ControllerAxis axis, int16_t value) noexcept = 0;
};

void app_run(std::unique_ptr<AppDelegate> app, Icon icon, int argc, char** argv, std::string_view configPath,
             Backend desiredBackend = Backend::Invalid, u32 msaa = 1, u16 aniso = 16,
...
[[nodiscard]] u32 get_which_controller_for_player(int32_t index) noexcept;
[[nodiscard]] int32_t get_controller_player_index(u32 which) noexcept;
void set_controller_player_index(u32 which, int32_t index) noexcept;
[[nodiscard]] bool is_controller_gamecube(u32 which) noexcept;
[[nodiscard]] bool controller_has_rumble(u32 which) noexcept;
void controller_rumble(u32 which, u16 low_freq_intensity, u16 high_freq_intensity,
                       u32 duration_ms = 0) noexcept;

u32 get_controller_count() noexcept;
[[nodiscard]] std::string get_controller_name(u32 which) noexcept;
} // namespace aurora
