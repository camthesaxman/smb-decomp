...
static volatile u32 salLastTick;
static volatile u32 salDspInitIsDone;
static volatile u16 hwIrqLevel;
static volatile int oldState;
static u8 salAIBufferIndex;

int salGetStartDelay(void);

void salCallback(void)
{
    salAIBufferIndex = (salAIBufferIndex + 1) % 4;
    AIInitDMA((uintptr_t)salAIBufferBase + 0x80000000 + (salAIBufferIndex * 0x280), 0x280);
...
static void dspInitCallback(void *task)
{
    salDspIsDone = 1;
    salDspInitIsDone = 1;
}
...
u32 salInitAi(void (*arg0)(void), int arg1, int *arg2)
{
    ...
    salAIBufferBase = buffer;
    if (buffer != NULL)
    {
        memset(salAIBufferBase, 0, 0xA00);
        DCFlushRange(salAIBufferBase, 0xA00);
        userCallback = arg0;
        salAIBufferIndex = 1;
        salLogicIsWaiting = 0;
        salDspIsDone = 1;
...
{
    u8 var = (salAIBufferIndex + 2) % 4;
    return (u8 *)salAIBufferBase + (var * 0x280);
}
...
static u16 dspSlaveLength = sizeof(dspSlave);
...
u32 salInitDsp(int unused)
{
    ...
    dsp_task.dram_mmem_addr = dram_image;
    dsp_task.dram_length = sizeof(dram_image);
...
void salCtrlDsp(void *arg0)
{
    ...
    salBuildCommandList(arg0, salGetStartDelay());
    msg = dspCmdList;
    salDspIsDone = 0;
    PPCSync();
