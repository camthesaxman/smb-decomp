...
void ev_info_init(void)
{
    ...
    if (modeCtrl.levelSetFlags & 1)
        infoWork.timerCurr = q_get_stage_time_limit();
...
void ev_info_main(void)
{
    ...
    for (i = 0; i < spritePoolInfo.unk8; i++, ball++, r23++)
    {
        ...
        case GAMETYPE_MAIN_COMPETITION:
            ...
            {
                q_get_replay_info(lbl_80250A68.unk0[ball->playerId], &spC8);
                if (!(spC8.flags & 1))
                    continue;
            }
            if (ball->flags & BALL_FLAG_24)
        ...
        default:
            ...
            {
                q_time_over_all_competition_mode_balls();
                func_800245E4(ball, goalId, sp64);
    ...
    if (!(infoWork.flags & INFO_FLAG_05)
     ...
     && (q_unkInputArr1[0] & PAD_BUTTON_X) && (q_unkInputArr1[0] & PAD_BUTTON_Y))
    {
        ...
        if ((q_unkInputArr1[0] & PAD_BUTTON_UP)
         || (q_unkInputArr1[0] & PAD_BUTTON_RIGHT)
         || (q_unkInputArr1[0] & PAD_BUTTON_DOWN))
        {
            // fake match
            goal = ((volatile struct StageAnimGroup *)&decodedStageLzPtr->animGroups[0])->goals;
            for (i = 0; i < decodedStageLzPtr->animGroups[0].goalCount; i++, goal++)
            {
                if ((q_unkInputArr1[0] & PAD_BUTTON_UP) && goal->type == 'R')
                {
                    goalId = i;
                    break;
                }
                if ((q_unkInputArr1[0] & PAD_BUTTON_RIGHT) && goal->type == 'G')
                {
                    goalId = i;
                    break;
                }
                if ((q_unkInputArr1[0] & PAD_BUTTON_DOWN) && goal->type == 'B')
                {
                    goalId = i;
                    break;
                }
        ...
        if (!(infoWork.flags & INFO_FLAG_BONUS_STAGE) && (q_unkInputArr1[0] & PAD_BUTTON_LEFT))
            infoWork.unk22 = 10;
        ...
        if (!(infoWork.flags & INFO_FLAG_BONUS_STAGE))
        {
            ...
            q_break_goal_tape(infoWork.goalEntered, &sp8);
            ball->unk12A = infoWork.timerCurr;
            q_play_sound(0x16);
        }
    }
...
void q_time_over_all_competition_mode_balls(void)
{
    if (infoWork.flags & INFO_FLAG_BONUS_STAGE)
