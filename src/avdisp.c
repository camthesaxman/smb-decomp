...
static inline void *skip_mesh(struct GMAShape *mesh)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (mesh->g_vtxRenderFlags & (1 << i))
            ptr += mesh->dispListSizes[i];
    }

    // TODO: what is this?
    if (mesh->g_vtxRenderFlags & 0xC)
    {
        u32 *r4 = (u32 *)ptr;
        ptr += 32;
        ptr += r4[2];
...
#pragma force_active on
void set_mesh_render_flags_in_model(struct GMAModel *model, u32 flags)
{
    struct GMAShape *meshPtr = OFFSET_TO_PTR(model, model->headerSize);
...
static inline struct GMAShape *draw_mesh_deferred(struct GMAModel *model, struct GMAShape *mesh, struct GMASampler *mtrl)
{
    ...
    if (mesh->unk14 != 0xFF)
        entry = ord_tbl_get_entry_for_pos_offset_index(&mesh->boundSphereCenter, -1);
    else
        entry = ord_tbl_get_entry_for_pos(&mesh->boundSphereCenter);
...
{
    struct GMAShape *mesh = OFFSET_TO_PTR(model, model->headerSize);
    struct GMASampler *mtrl = OFFSET_TO_PTR(model, 0x40);
    int i;
...
void init_material(struct GMASampler *mtrl, struct TPLTextureHeader *texHdr, struct TPL *tpl)
{
    u8 maxLOD;
    int wrapS;
    int wrapT;
...
{
    struct GMASampler *materials;
    struct GMAShape *mesh;
    int i;

    if (tpl == NULL)
        model->samplerCount = 0;
    materials = (void *)model->samplers;
    ...
    {
        if (tpl == NULL)
            mesh->tevStageCount = 0;
        if (model->flags & (GCMF_SKIN|GCMF_EFFECTIVE))
        {
            struct GMAShape *r3 = mesh;
            mesh++;
...
struct GMAShape *draw_model_8008F914(struct GMAModel *model, struct GMAShape *mesh, struct GMASampler *mtrl)
{
    ...
    if (bvar)
    {
        ...
        if ((mesh->g_vtxRenderFlags & 0xC) != 0)
        {
...
void *draw_mesh_reflection_maybe(struct GMAShape *mesh, void *mtrl, struct UnkStruct29 *c, u8 *d)
{
    int i;
    int r30;
    u8 *r29 = (u8 *)c + c->unk8;
...
void func_8008FE44(struct GMAModel *model, struct GMAShape *mesh)
{
    lbl_802B4ECC.unk0 = 1;
    if (model->flags & (GCMF_SKIN|GCMF_EFFECTIVE))
        mesh = (struct GMAShape *)((u8 *)mesh + 0x20);
    if (lbl_802F2108 != 0)
    ...
    lbl_802B4ECC.unk2 = 0;
    lbl_802B4ECC.g_tevColorInD = 15;
    lbl_802B4ECC.g_tevAlphaInD = 7;
    lbl_802B4ECC.unk3 = -1;
    lbl_802B4ECC.unk4 = -1;
    lbl_802B4ECC.unk5 = -1;
    ...
    lbl_802B4ECC.unk1 = -1;
    lbl_802B4ECC.g_tevStagesFilled[0] = -1;
    lbl_802B4ECC.g_tevStagesFilled[1] = -1;
    lbl_802B4ECC.g_tevStagesFilled[2] = -1;
    ...
    mathutil_mtxA_tf_point(&model->boundSphereCenter, &lbl_802B4ECC.unk58);
    lbl_802B4ECC.unk58.z -= model->boundSphereRadius;
...
void g_build_tev_material(struct GMAShape *shape, struct GMASampler *modelSamplers)
{
    ...
    if (shape->flags & 1)
    {
        ...
        else
        {
            ...
            g_tevColorInD = GX_CC_C0;
            g_tevAlphaInD = GX_CA_A0;
            //to lbl_800908D4
        }
    }
    //lbl_80090814
    ...
    //lbl_8009093C
    if (lbl_802B4ECC.g_tevColorInD != g_tevColorInD)
    {
        lbl_802B4ECC.g_tevColorInD = g_tevColorInD;
        g_isNewTevColorInD = 1;
    }
    if (lbl_802B4ECC.g_tevAlphaInD != g_tevAlphaInD)
    {
        lbl_802B4ECC.g_tevAlphaInD = g_tevAlphaInD;
        g_isNewTevAlphaInD = 1;
    }
    if (shape->flags & 0x80)
    {
        if (lbl_802B4ECC.unk1 != 0 || g_isNewTevColorInD != 0 || g_isNewTevAlphaInD != 0)
        {
            ...
            GXSetTevColorIn_cached(sp7C.tevStage, GX_CC_ZERO, GX_CC_ZERO, GX_CC_ZERO, g_tevColorInD);
            GXSetTevColorOp_cached(sp7C.tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_TRUE, GX_TEVPREV);
            GXSetTevAlphaIn_cached(sp7C.tevStage, GX_CA_ZERO, GX_CA_ZERO, GX_CA_ZERO, g_tevAlphaInD);
        ...
        else
        {
            if (g_isNewTevColorInD != 0) // Impossible
            {
                if (g_isNewTevColorInD != 0)
                    GXSetTevColorIn_cached(sp7C.tevStage, 15, 15, 15, g_tevColorInD);
                //lbl_80090A40
                if (g_isNewTevAlphaInD != 0)
                    GXSetTevAlphaIn_cached(sp7C.tevStage, 7, 7, 7, g_tevAlphaInD);
            }
        }
        //lbl_80090A60
        lbl_802B4ECC.g_tevStagesFilled[0] = -1;
    ...
    else
    {
        ...
        u32 *r17 = lbl_802B4ECC.g_tevStagesFilled;
        u32 r16;
        ...
        while (tevStageCounter > 0)
        {
            ...
            {
                GXLoadTexObj_cached(sampler->texObj, sp7C.g_someTexmapId);
            ...
            if (r16 == 0)
            {
                if (g_isNewTevColorInD != 0)
                {
                    func_80091500(&sp7C, g_tevColorInD, g_tevAlphaInD);
                    g_isNewTevColorInD = 0;
                }
                func_80091564(&sp7C);
                //to lbl_80090D3C
            ...
            else if (r16 & 0x2000)
            {
                if (g_isNewTevColorInD != 0)
                {
                    func_8009167C(&sp7C, g_tevColorInD, g_tevAlphaInD);
                    g_isNewTevColorInD = 0;
                }
                func_800916E0(&sp7C);
                //to lbl_80090D3C
            ...
            else if (r16 & 0x2)
            {
                if (g_isNewTevColorInD != 0)
                {
                    func_80091CA8(&sp7C, g_tevColorInD, g_tevAlphaInD);
                    g_isNewTevColorInD = 0;
                }
                func_80091D0C(&sp7C);
                //to lbl_80090D3C
            ...
            else if (r16 & 1)
            {
                ...
                if (g_isNewTevColorInD != 0)
                {
                    func_80091878(&sp7C, g_tevColorInD, g_tevAlphaInD);
                    g_isNewTevColorInD = 0;
                }
                func_800918DC(&sp7C);
                //to lbl_80090D3C
            ...
            else
            {
                ...
                if (g_isNewTevColorInD != 0)
                {
                    func_80091B1C(&sp7C, g_tevColorInD, g_tevAlphaInD);
                    g_isNewTevColorInD = 0;
                }
                func_80091B88(&sp7C);
            }
        ...
        while (tevStageCounter > 0)
        {
            ...
            *r17 = r16;
            ...
            {
                GXLoadTexObj_cached(r3->texObj, sp7C.g_someTexmapId);
                *g_someSamplerIdx = *samplerIdx;
            }
            if (r16 == 0)
            {
                func_80091404(&sp7C, g_tevColorInD, g_tevAlphaInD, r19);
                func_80091564(&sp7C);
            }
            else if (r16 & 0x2000)
            {
                func_80091580(&sp7C, g_tevColorInD, g_tevAlphaInD, r19);
                func_800916E0(&sp7C);
            }
            else if (r16 & 0x2000)  // same thing
            {
                func_80091BA4(&sp7C, g_tevColorInD, g_tevAlphaInD, r19);
                func_80091D0C(&sp7C);
            }
            //lbl_80090E54
            else if (r16 & 1)
            {
                ...
                func_800916FC(&sp7C, g_tevColorInD, g_tevAlphaInD, r19);
                func_800918DC(&sp7C);
            }
            else
            {
                ...
                func_800918F8(&sp7C, g_tevColorInD, g_tevAlphaInD, r19);
                func_80091B88(&sp7C);
            }
            sp7C.g_someTexmapId++;
            r19++;
