...
void ev_stobj_main(void)
{
    ...
    phi_r27 = poolInfo.unk2C;
    stobj = stobjInfo;
    for (i = poolInfo.unk28; i > 0; i--, phi_r27++, stobj++)
    {
        if (*phi_r27 != 0)
        {
            if (*phi_r27 == 3)
...
void stobj_draw(void)
{
    ...
    phi_r29 = poolInfo.unk2C;
    phi_r25 = 0;
    stobj = stobjInfo;
    for (i = poolInfo.unk28; i > 0; i--, phi_r29++, stobj++)
    {
        if (*phi_r29 != 0)
        {
...
s16 spawn_stobj(struct Stobj *arg0)
{
    ...
    temp_r3 = pool_alloc(poolInfo.unk20, 1);
    if (temp_r3 < 0)
        return -1;
...
static void stobj_bumper_draw(struct Stobj *stobj)
{
    ...
    if (stobj->unk48 > 1.0f)
    {
        ...
        nl2ngc_draw_model_unsorted(naomiCommonObj->models[0x2B]);
