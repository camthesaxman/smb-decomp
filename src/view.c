...
void view_init_stage_anim(void)
{
    ...
    stageViewInfo->unk40 = lbl_80206DEC.q_stageTimer;
...
void view_animate_stage(void)
{
    ...
    lbl_80206DEC.q_stageTimer = stageViewInfo->frameCounter;
    ...
    if (dynamicStageParts != NULL)
    {
        ...
        while (dyn->modelName != NULL)
        {
            ...
            q_apply_func_to_naomi_model_vertices(dyn->tempModel, dyn->posNrmTexFunc, dyn->posColorTexFunc);
            dyn++;
        }
    }
}
...
{
    lbl_80206DEC.q_stageTimer = stageViewInfo->unk40;
}
...
void draw_stage_geometry(void)
{
    ...
    q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, 10));
    func_8000E3BC();
...
void draw_stage_objects(void)
{
    ...
    for (i = 0; i < animGroupCount; i++)
    {
        ...
        for (j_r25 = 0; j_r25 < goalCount; j_r25++, goal++)
        {
            ...
            mathutil_mtxA_push();
            q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, 0x1D));
            mathutil_mtxA_translate_xyz(-0.45f, 0.0f, 0.0f);
            q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, 0x1D));
            mathutil_mtxA_pop();

            q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, 0x13));
