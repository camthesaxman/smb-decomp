...
void q_clear_mot_transforms(struct JointBoneThing *b);
void q_load_mot_transforms(struct MotDat *dat, struct JointBoneThing *b);
void q_read_transform_list_from_dat(struct MotDat *a, struct MotionTransform *b);
...
void func_80034360(struct JointBoneThing *a, u16 b)
{
    ...
    q_clear_mot_transforms(a);
    dat = motDat[b - 1];
    q_load_mot_transforms(&dat, a);
}
...
void q_load_mot_transforms(struct MotDat *dat, struct JointBoneThing *b)
{
    ...
    for (i = 0; b->flags != 0; i++, b++)
    {
        if (r30->unk0 == (u8)i)
        {
            ...
            if (r31 & (1 << 8))
                q_read_transform_list_from_dat(dat, &b->transforms[0]);
            if (r31 & (1 << 7))
                q_read_transform_list_from_dat(dat, &b->transforms[1]);
            if (r31 & (1 << 6))
                q_read_transform_list_from_dat(dat, &b->transforms[2]);
            if (r31 & (1 << 5))
                q_read_transform_list_from_dat(dat, &b->transforms[3]);
            if (r31 & (1 << 4))
                q_read_transform_list_from_dat(dat, &b->transforms[4]);
            if (r31 & (1 << 3))
                q_read_transform_list_from_dat(dat, &b->transforms[5]);
            if (r31 & (1 << 2))
                q_read_transform_list_from_dat(dat, NULL);
            if (r31 & (1 << 1))
                q_read_transform_list_from_dat(dat, NULL);
            if (r31 & (1 << 0))
                q_read_transform_list_from_dat(dat, NULL);
...
void q_read_transform_list_from_dat(struct MotDat *dat, struct MotionTransform *transform)
{
    u8 r5 = dat->unk8->unk0;
    int r6;
    u8 *r4;
...
int init_ape_model_info(char *datname, char *labelname, char *sklname, char *infoname)
{
    ...
    q_read_dvd_file(&file, motLabel, size, 0);
    DVDClose(&file);
    q_motAnimCount = *motLabel;
    motLabel++;
    ...
    if (!DVDOpen(datname, &file))
        return 0;
    if (q_read_dvd_file(&file, lzssHeader, 32, 0) < 0)
    ...
    if (q_read_dvd_file(&file, compressed, compSize, 0) < 0)
    ...
    q_read_dvd_file(&file, motSkeleton, size, 0);
    DVDClose(&file);
