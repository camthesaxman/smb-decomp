...
void q_interpolate_joint_motion(struct JointBoneThing *a, const struct Struct80034F5C_3 *b, const struct Struct80034F5C_2 *c, float e, u32 d)
{
    u32 flags;
    ...
    while (flags != 0)
    {
        ...
        {
            if (d != 0)
                q_interp_pos_motion(r30, &a[c->unk2], d, e);
            else
                q_interp_pos_motion(r30, r30, d, e);
            c++;
        }
        if (flags & (1 << 3))
        {
            if (d != 0)
                q_interp_rot_motion(r30, &a[b->unk2], b, d, e);
            else
                q_interp_rot_motion(r30, r30, b, d, e);
            b++;
        }
        r30++;
        flags = r30->flags;
...
void q_interp_pos_motion(struct JointBoneThing *a, struct JointBoneThing *b, u32 c, float d)
{
    ...
    if (r3->unk0 != 0)
    {
        b->unk1C0.x = q_interp_skelanim_value_maybe(r3, d);
        if (c != 0)
    ...
    if (r3->unk0 != 0)
        b->unk1C0.y = q_interp_skelanim_value_maybe(r3, d);
    else
        b->unk1C0.y = 0.0f;
    r3++;

    if (r3->unk0 != 0)
        b->unk1C0.z = q_interp_skelanim_value_maybe(r3, d);
    else
        b->unk1C0.z = 0.0f;

}

void q_interp_rot_motion(struct JointBoneThing *a, struct JointBoneThing *b, const struct Struct80034F5C_3 *c, u32 d, float e)
{
    float f31;
    ...
    if (sub->unk0 != 0)
    {
        float f1 = q_interp_skelanim_value_maybe(sub, e);
        if (d != 0)
...
float q_interp_skelanim_value_maybe(struct MotionTransform *transform, float t)
{
    float ret;
    float dummy1;
    float dummy2;
    ...
    while (r31 < endSomething)
    {
        ...
        r31++;
        q_skelanim_seek_next(transform);
    }
    ...
    default:
        if (transform->unk1 < endSomething)
            q_skelanim_seek_prev(transform);
        // fall through
    case 1:
    case 2:
    ...
    case 3:
        ...
        q_skelanim_seek_prev(&sp10);
        sp2C.x = *sp10.unk4;
        read_transform_values(&sp10, &dummy1, &sp2C.z, &sp2C.y);
        ret = q_crazy_interpolation_stuff(&sp2C, &sp20, t);
        break;
    }
    transform->unk1 = r31;
    return ret;
...
float q_crazy_interpolation_stuff(Vec *a, Vec *b, float c)
{
...
void q_skelanim_seek_next(struct MotionTransform *transform)
{
    transform->unk4++;
...
void q_skelanim_seek_prev(struct MotionTransform *transform)
{
    transform->unk4--;
    transform->numComponents--;
    transform->values -= *transform->numComponents;
