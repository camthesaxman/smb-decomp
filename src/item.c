...
void ev_item_main(void)
{
    ...
    item = itemPool;
    for (r31 = poolInfo.itemPoolUpperBound; r31 > 0; r31--, status++, item++)
    {
        if (*status != 0)
        {
            if (item->unkC != 0)
...
void ev_item_dest(void)
{
    ...
    r27 = poolInfo.itemStatusList;
    item = itemPool;
    for (r29 = poolInfo.itemPoolUpperBound; r29 > 0; r29--, r27++, item++)
    {
        if (*r27 != 0)
        {
            itemDestroyFuncs[item->type](item);
...
{
    struct Item *r31;
    int r30 = pool_alloc(poolInfo.unk10, 1);
...
void item_draw_shadows(void)
{
    ...
    r26 = poolInfo.itemStatusList;
    item = itemPool;
    for (r28 = poolInfo.itemPoolUpperBound; r28 > 0; r28--, r26++, item++)
    {
        if (*r26 == 0
         || !(item->flags & (1 << 5))
...
void func_800689B4(int a)
{
    ...
    r26 = poolInfo.itemStatusList;
    item = itemPool;
    for (r29 = poolInfo.itemPoolUpperBound; r29 > 0; r29--, r26++, item++)
    {
