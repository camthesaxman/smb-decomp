...
void q_choose_ape_anim(struct Ape *ape, float b)
{
    struct Ball *ball = &ballInfo[ape->ballId];
    float f31 = b;
    int r29;
    ...
    else
    {
        if (ape->flags & (1 << 8))
        {
            r29 = 1;
            r27 = 6;
            q_switch_ape_character_lod_maybe(ape, 0);
        }
    ...
    if (r29 != 1 || gameSubmode == SMD_GAME_READY_MAIN)
        ape->unk54 = 0;
    q_set_ape_anim(ape, r29, r28, r27, f31);
}
...
void ev_ball_init(void)
{
    ...
    for (i = 0; i < 4; i++,  ball++, r21++)
    {
        ...
        func_8004C754();
        func_8008BA24(1);
        ape = q_make_ape(playerCharacterSelection[i]);
        ball->ape = ape;
        ape->unk74 = 0;
        ...
        case GAMETYPE_MINI_FIGHT:
            ...
            {
                q_switch_ape_character_lod_maybe(ape, 2);
                lbl_802F1F0C |= 1 << (ape->charaId * 2 + 1);
            }
            else
            {
                q_switch_ape_character_lod_maybe(ape, 1);
                lbl_802F1F0C |= 1 << (ape->charaId * 2);
            }
            break;
        case GAMETYPE_MINI_RACE:
...
{
    struct Ape *ape = q_make_ape(character);
...
void ball_draw(void)
{
    ...
    for (i = 0; i < spritePoolInfo.unk8; i++, ball++, r27++)
    {
        ...
        q_nl2ngc_set_scale(ball->modelScale);

        if (dipSwitches & (DIP_STCOLI | DIP_TRIANGLE))
        {
            q_call_draw_model_with_alpha_deferred(NLOBJ_MODEL(naomiCommonObj, ball->oldModelId), 0.3f);
        }
        else
        {
            q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, ball->oldModelId));
        }
        ...
        if (envFunc != NULL)
        {
            q_avdisp_set_some_func_1(envFunc);
        ...
        mathutil_mtxA_scale_s(ball->modelScale);
        q_nl2ngc_set_scale(ball->modelScale);
        q_call_draw_naomi_model_and_do_other_stuff(NLOBJ_MODEL(naomiCommonObj, NLMODEL_common_BALL_EDGE));
        mathutil_mtxA_pop();
...
void q_ball_shadow_something_1(void)
{
    ...
    for (i = 0; i < spritePoolInfo.unk8; i++, ball++, r26++)
    {
        ...
        q_init_shadow_stuff_probably(&sp18);
    }
}

void q_ball_shadow_something_2(void)
{
...
void give_bananas(int bananas)
{
    ...
    case GAMETYPE_MAIN_NORMAL:
        ...
        if (ball->bananas >= 100)
        {
            ...
            q_play_sound(0x2852);  // play 1-up sound?
        }
        break;
    case GAMETYPE_MAIN_COMPETITION:
...
void q_ball_init_1(struct Ball *ball)
{
    struct Ball backup;
...
void q_ball_init_2(struct Ball *ball)
{
    struct Ball backup;
...
void ball_func_19(struct Ball *ball)
{
    ...
    q_play_sound(ball->lives == 1 ? 0x51 : 0x1D);
    q_play_sound(0x15);
    ball->state = 20;
}
...
static inline void func_8003CDC0_sub(struct Ball *ball)
{
    ...
    if ((lbl_80206BE0[ball->playerId] % 30) == 1)
        q_play_sound(lbl_801179D4[(rand() >> 12) & 7]);
}
