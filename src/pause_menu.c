...
int should_open_pause_menu(void)
{
    ...
    if (dipSwitches & DIP_DEBUG)
    {
        if (!(analogButtonInfo[0][0] & PAD_BUTTON_A)
         ...
         && lbl_802F1ED8 == 0
         && !(gamePauseStatus & 8)
         && (q_unkInputArr1[2] & PAD_BUTTON_START))
            return TRUE;
    }
    else
    {
        if (!(gamePauseStatus & 8) && (q_unkInputArr1[2] & PAD_BUTTON_START))
            return TRUE;
    }
    return FALSE;
}

void q_open_pause_menu(struct Sprite *menuSprite)
{
    int i;
...
void q_handle_pause_menu_navigation(struct Sprite *menuSprite)
{
...
void q_pause_menu_load_how_to_play(struct Sprite *menuSprite)
{
    lbl_802F1B98 = 4;
    if (menuSprite != NULL)
...
void q_activate_pause_menu_item(struct Sprite *menuSprite)
{
    gamePauseStatus &= ~(1 << 3);
    switch (pauseMenuState.menuType)
    {
    case PAUSEMENU_CONT_HOW_EXIT:
        ...
        case 0:  // "Continue"
            ...
            break;
        case 1:  // "How to play"
            q_pause_menu_load_how_to_play(menuSprite);
            break;
        case 2:  // "Exit game"
            lbl_802F1B98 = 1;
            func_80012434(-1);
...
void q_menu_input_game_notdebug(void)
{
    ...
    if (should_open_pause_menu())
        q_open_pause_menu(menuSprite);
    else
    {
        if (gamePauseStatus & (1 << 3))
        {
            if (!(pauseMenuState.unk4 & 1))
                q_handle_pause_menu_navigation(menuSprite);
            ...
            {
                if (--pauseMenuState.unk0 == 0)
                    q_activate_pause_menu_item(menuSprite);
            }
            else
