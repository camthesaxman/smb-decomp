...
void init_gamedata_file(void)
{
    ...
    if (q_read_dvd_file(&file, buffer, sizeof(memcardGameData->bannerAndIcon), 0) == 0)
...
void init_replay_file_data(void)
{
    ...
    if (q_read_dvd_file(&file, buffer, 0x800, 0) == 0)
...
void save_sequence(void)
{
    ...
    case MC_STATE_OPEN_FILE:
        ...
        break;
    case 9:
        if ((q_unkInputArr1[2] & PAD_BUTTON_LEFT)
         || (q_unkInputArr2[2] & PAD_BUTTON_LEFT))
        {
            if (lbl_802F21B1 == 0)
                func_8002B5C8(0x6C);
            lbl_802F21B1 = 1;
        }
        if ((q_unkInputArr1[2] & PAD_BUTTON_RIGHT)
         || (q_unkInputArr2[2] & PAD_BUTTON_RIGHT))
        {
            if (lbl_802F21B1 == 1)
                func_8002B5C8(0x6C);
            lbl_802F21B1 = 0;
...
void ev_memcard_main(void)
{
    ...
    if (memcardInfo.statusFlags & MC_STATUS_ERROR)
    {
        ...
        if (q_unkInputArr1[2] & PAD_BUTTON_B)
        {
            func_8002B5C8(0x6B);
            memcardInfo.unk42 = 0;
            memcardInfo.statusFlags &= ~MC_STATUS_ERROR;
...
void draw_memcard_msg(struct MemCardMessage *msg, float x, float y)
{
    ...
    {
        float lineWidth = q_get_text_width(msg->lines[i].str);
        if (lineWidth > f30)
            f30 = lineWidth;
    }
    if (r27 > 0)
...
void func_800A4E70(void)
{
    ...
    q_store_gamedata(memcardGameData);
...
void func_800A4F04(void)
{
    ...
    q_load_gamedata(memcardGameData);
