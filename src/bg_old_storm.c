...
void bg_old_storm_main(void)
{
    ...
    mathutil_vec_set_len(&spDC, &effect.pos, 3.5 * RAND_FLOAT());
    effect.pos.x += spD0.x;
    effect.pos.y += spD0.y;
    effect.pos.z += spD0.z;
    effect.vel.x = 0.0f;
    ...
    if (gameSubmode == SMD_GAME_CONTINUE_MAIN)
    {
        ...
        effect.unk70.y = 1.0f;
        effect.unk70.z = 0.0f;
        mathutil_vec_to_euler_xy(&sp8.normal, &effect.rotX, &effect.rotY);
    ...
    else if ((u32)raycast_stage_down(&spD0, &sp8, &spDC) != 0)
    {
        memset(&effect, 0, sizeof(effect));
        effect.type = ET_RAINDROP_RIPPLE;
        effect.pos.x = sp8.pos.x + 0.04 * sp8.normal.x;
        effect.pos.y = sp8.pos.y + 0.04 * sp8.normal.y;
        effect.pos.z = sp8.pos.z + 0.04 * sp8.normal.z;
