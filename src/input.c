...
u16 q_unkInputArr1[6];
u16 q_unkInputArr2[6];
...
FORCE_BSS_ORDER(q_unkInputArr1);
FORCE_BSS_ORDER(q_unkInputArr2);
...
void func_80025640(void)
{
    ...
    case 5:
        for (i = 0; i < 4; i++)
        {
            ...
            for (j = 0; j < 5; j++)
            {
                q_unkInputArr1[j] |= controllerInfo[i].unk0[j].button;
                q_unkInputArr2[j] |= analogButtonInfo[i][j];
            }
        }
        break;
    default:
        ...
        for (i = 0; i < 4; i++, r27++)
        {
            ...
            for (j = 0; j < 5; j++)
            {
                q_unkInputArr1[j] |= controllerInfo[lbl_80206BD0[i]].unk0[j].button;
                q_unkInputArr2[j] |= analogButtonInfo[lbl_80206BD0[i]][j];
            }
        }
        break;
    }
