...
void animate_bg_models(struct StageBgModel *bgModels, int bgModelCount, float timeSeconds)
{
    ...
    for (i = 0; i < bgModelCount; i++, bgModels++)
    {
        ...
        if (bgModels->flags & (1 << 6))
            timeSecondsLooped = lbl_80206DEC.q_stageTimer / 60.0;
    ...
    for (i = 0; i < bgModelCount; i++, bgModels++)
    {
        ...
        if ((lbl_801EEC90.unk0 & (1 << 2))
         && func_8000E444(&model->boundSphereCenter) < -(f29 * model->boundSphereRadius))
            continue;
        if (q_test_scaled_sphere_in_frustum(&model->boundSphereCenter, model->boundSphereRadius, f29) == 0)
            continue;
        ...
        if (backgroundInfo.unk90 != 0 && (bgModels->flags & (1 << 24)))
            q_avdisp_set_some_func_1(backgroundInfo.unk90);
...
void bg_billiards_init(void)
{
    ...
    for (i = 0; i < decodedStageLzPtr->bgModelsCount; i++, r29++)
    {
        ...
        while (r27->q_bgLightGroupId != -1)
        {
            ...
            if (matched)
            {
                r29->flags |= r27->q_bgLightGroupId << 28;
                break;
            }
            r27++;
        }
...
void q_search_bg_models(struct BGModelSearch *searchList, int (*func)(int, struct GMAModelEntry *))
{
    int i;
    int j;
...
void q_search_bg_models_from_list(struct StageBgModel *bgModels, int count, struct BGModelSearch *searchList, Func800567DC func)
{
    int i;
    int j;
    int r25 = 1;
