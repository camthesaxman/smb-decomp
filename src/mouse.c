...
void ev_mouse_main(void)
{
    ...
    if (
        ...
        _lbl_802BA190->posHorizontal = _lbl_802BA190->posHorizontal + (controllerInfo[0].unk0[0].stickX * 0.1);
        *ptr_posVertical = *ptr_posVertical + (-controllerInfo[0].unk0[0].stickY * 0.1);

        // Horizontal Check
        if (_lbl_802BA190->posHorizontal < HORIZONTAL_MIN) {
            _lbl_802BA190->posHorizontal = HORIZONTAL_MIN;
        }
        if (_lbl_802BA190->posHorizontal > HORIZONTAL_MAX) {
            _lbl_802BA190->posHorizontal = HORIZONTAL_MAX;
        }

        // Vertical Check
        if (*ptr_posVertical < VERTICAL_MIN) {
            *ptr_posVertical = VERTICAL_MIN;
        }
        if (*ptr_posVertical > VERTICAL_MAX) {
            *ptr_posVertical = VERTICAL_MAX;
        }

        _unk08 = &_lbl_802BA190->unk08;
        *_unk08 = *_unk08 - _lbl_802BA190->posHorizontal;
        _unk0a = &_lbl_802BA190->unk0a;
        *_unk0a = *ptr_posVertical - *_unk06;
        
        if ( (controllerInfo[0].unk0[2].button & PAD_BUTTON_A) != 0 ) {
            // if press "A button"
            ptr_spriteIdx = &_lbl_802BA190->spriteIdx;
